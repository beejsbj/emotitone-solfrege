...
| CodeStrip | Unique | **Under review; next** | `components/uniques/CodeStrip.vue` and its real specimen hold the proposed visual grammar; production `LiveStrip.vue` holds the functionality CodeStrip succeeds and currently duplicates notation presentation. The specimen's obsolete `shape="glyph"` call is ignored by accepted `Note.vue`, so each note renders at the full 56x88px medium fallback geometry and makes the claimed 36px row about 111px tall at the inspected guide width | Add `glyph` to Note's proportion family with host-controlled responsive scale and preserve production progressive fill; then settle CodeStrip's remaining density and states before formalization |
