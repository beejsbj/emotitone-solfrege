...
- Button is accepted, formalized, mounted as a real specimen, and adopted in production. A user-led correction is active for its visual finish: remove the faux inset outline, restore canonical brass finishes, enlarge the icon, and inspect full geometry-token recipes without promoting them into production variants.
...
| Button | Primitive | **Accepted; visual correction active** | `components/primatives/Button.vue` is authoritative; the real specimen imports it; production Config, Instrument Selector, Keyboard action bar, Pattern Card, and inactive sequencer placeholders consume it. Former IconButton and Knob Button sources are removed | Remove faux outlines, use recognizable centered icons, make sheen + glow the production brass default, and display four brass finishes plus specimen-only geometry recipes before re-verifying lineage |
