...
| **Instrument Picker** | Instrument-selection content inside the shared top Drawer | Defined, authoritative in production `InstrumentSelector.vue`, mounted directly as the focused real guide specimen, and production-adopted. Every visible choice is a real Ivory Sticker inside an invisible native button: outline for available/ready, dimmed outline for cold, and filled for current/warming. Current/warming lead each bank, already-warmed sounds follow before cold sounds, and the shared instrument resolver supplies family-specific icons to both Drawer handles and Stickers with an AudioWaveform fallback. The rail contains only actual category banks—there is no aggregate All destination—and opens on the selected instrument's bank; Search still spans every bank. It inherits direct Tabs-rail scrolling and paired horizontal page motion from the shared top-panel seam | Preserve the shared Drawer/Tabs frame, cross-bank search, category behavior, stable alphabetical order within each readiness tier, controlled/store selection flows, warmup/error handling, audio, accessibility semantics, and Sticker-owned visible choice material; neither brass nor Badge belongs here |
