...
| CodeStrip Bar | Compound | **Accepted and closed** | `components/compounds/CodeStripBar.vue` is authoritative and is shared by production `DrawerKeyboard.vue` and real `style-guide/compounds/CompoundCodeStripBar.vue`. It composes accepted Buttons and an authoritative CodeStrip through its public unframed seam | Preserve the continuous Ink-2 rail, zero gaps/frame/inset, component name, other CodeStrip contexts, and action behavior |
...
- CodeStrip Bar owns one continuous Ink-2 rail behind all four members. It has no outline, outer padding, or inter-item gaps. Its CodeStrip consumer is dense, has zero content inset, and uses CodeStrip's explicit unframed transparent treatment so no inner rectangle divides the bar.
