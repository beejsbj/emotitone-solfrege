...
| Card | Burooj selected the first extracted CardShell iteration as the only visual recipe worth preserving; compact, borderless, and light-inversion variants are rejected. The guide-only source still requires editorial title/body anatomy and has no production consumer | Inspect the selected recipe with a 4px Ink spine and flexible content boundary, then accept or adjust it before formalization and Pattern Card adoption |
