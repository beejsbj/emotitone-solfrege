...
- The legacy user-facing `keyboard.keyGaps` (`none | small | medium`) control is rejected. Keyboard will own one stable design-system spacing recipe across families and reloads; choose its exact value during the full specimen check at the provisional 44px minimum and larger supported sizes.
