...
| Tabs | Primitive | Defined, authoritative in `ChipTabs.vue`, mounted as a real guide specimen, and production-adopted through the Instrument and Config panel seam. One continuous chip moves over one rail; every unpinned instance shares a stable app-load edition cycling through the six accepted visible guide treatments, while explicit specimens remain pinned | Preserve the controlled content/state boundary in `Tabs`/`TabsContent`, the single presentation owner, page-wide edition consistency, internal overflow for long rails, and existing production panel behavior |
