...
The subsequent [architecture plan and measured recommendation](audio-architecture.md) consolidates playback into one context/transport and separates live input ownership from renderer choice. Its matched prepared-native comparison supports retaining the worklet default for rhythmic continuity, while exposing a native option that borrows original buffers without the extra PCM copy. See that record for the measured tradeoffs; the older baseline comparison above did not test this prepared native adapter.
