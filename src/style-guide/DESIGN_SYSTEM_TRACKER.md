...
- formalize and adopt the accepted CodeStrip action compound against the production CodeStrip.
...
| CodeStrip | Unique | **Accepted and production-adopted** | `components/uniques/CodeStrip/index.vue` is the sole public source and the actual production Strudel CodeMirror host. Its private `strudelExtension.ts` transforms only mini-notation event ranges, consumes native `showMiniLocations` for playback progress, and reveals the unchanged raw document on focus. Private `Sequence.vue` composes accepted Note and Chord and owns surfaced Rest plus duration/density presentation. The real `style-guide/uniques/UniqueCodeStrip.vue` specimen mounts this same CodeMirror path. No LiveStrip or whole-document visual layer remains | Define the adjacent compact Button compound; maintain source-range, mirror-focus, native-highlight, idle/Play, and behavior-preservation tests |
| CodeStrip action compound | Compound | **Accepted** | No authoritative source or real specimen exists yet. The current production `KeyboardActionBar.vue` is horizontally scrolling evidence to replace. The accepted anatomy is brass Play/Stop, flexible CodeStrip, then ink Backspace and ink Return; all actions use compact icon-only Button instances | Formalize one source, mount its real specimen, then replace the production action bar while preserving behavior |
