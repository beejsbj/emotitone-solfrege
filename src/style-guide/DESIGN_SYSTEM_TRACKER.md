...
| Brand Logo | Unique, compound-scale | The approved Centered Cluster is defined and authoritative in `BrandLogo.vue`: five smooth brand-colour circles sit behind a six-cut Ink/Ivory ET while nine accepted Marks scatter across an expanded field. The real guide specimen drives stacked, compact, mark-only, Ink, and Bone presentations through that source | Production adoption remains deferred to the Loading Screen composition gate; preserve the current `LoadingSplash.vue` and favicon until that composition is accepted |
...
| **Instrument Picker** | Instrument-selection content inside the shared top Drawer | Defined, authoritative in production `InstrumentSelector.vue`, mounted directly as the focused real guide specimen, and production-adopted. Every visible choice is a real Ivory Sticker inside an invisible native button: outline for available/ready, dimmed outline for cold, and filled for current/warming. Current/warming lead each group, already-warmed sounds follow before cold sounds, and the shared instrument resolver supplies family-specific icons to both Drawer handles and Stickers with an AudioWaveform fallback. It inherits direct Tabs-rail scrolling and paired horizontal page motion from the shared top-panel seam; the already-paid large All page remains inertly retained while the open picker visits another bank so reverse swipes do not remount it | Preserve the shared Drawer/Tabs frame, search and category behavior, stable alphabetical order within each readiness tier, controlled/store selection flows, warmup/error handling, audio, accessibility semantics, and Sticker-owned visible choice material; neither brass nor Badge belongs here. Retain only All, and only for the open panel lifetime |
| **Config Menu** | Configuration content inside the shared top Drawer | Defined, authoritative in production `ConfigPanel.vue`, mounted directly by the focused `/style-guide/config-menu` specimen, visually accepted, and production-adopted. The visible top-menu chrome now favors workhorse Ink and Ivory instead of a ladder of near-gray intermediates. Persistent settings are open, uniformly sized Ivory Knob fields under quiet groups; Brass sheen and glow are reserved for the global Visuals and active-section Boolean masters, whose 32px faces align with adjacent small Buttons while their labels sit below; local Section controls sit with the active section; global utilities remain in the top rail; momentary icon actions use Button; Scenes and Presets use Ivory Sticker faces inside native buttons; Badge stays absent. Labels wrap cleanly, displayed numeric precision is capped at two places, and the shared rail/content gestures expose offscreen tabs without compromising vertical or Knob gestures. Preserve the canonical grouped Blob schema, conditional visibility, persistence, and future UIBeat migration boundary |
...
PerformanceDeck follows Keyboard and the Pattern chain. Loading Screen follows its brand/lower-layer artifacts. Config Menu and Instrument Picker are closed. UIBeat implementation follows its protocol acceptance and must coordinate both Config and Stage hosts.
