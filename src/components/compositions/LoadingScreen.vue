...
<template>
  <section
    class="converged-loader loading-screen"
    :class="[
      `loading-screen--${mode}`,
      {
        'is-ready': isReady,
        'is-error': hasError && !isReady,
      },
    ]"
  ...
  >
    ...
    <main class="converged-loader__main">
      ...
      <div class="converged-loader__content">
        ...
        <div
          v-if="showMessages || needsAudioInteraction || (hasError && !isReady)"
          class="converged-loader__status"
          role="status"
          aria-live="polite"
        >
          ...
          <button
            v-if="needsAudioInteraction && !isReady"
            type="button"
            class="converged-loader__state-action"
          ...
          <button
            v-else-if="hasError && !isReady"
            type="button"
            class="converged-loader__state-action converged-loader__state-action--retry"
            @click="emit('retry')"
          >
            Retry
          </button>
        ...
        <div v-else class="converged-loader__status" aria-hidden="true">
          <div class="converged-loader__status-copy">
            <strong>&nbsp;</strong>
            <span>&nbsp;</span>
          </div>
...
.converged-loader__state-action,
.converged-loader__skip {
  min-height: 32px;
  border: 0;
  border-radius: 2px;
...
.converged-loader__state-action--retry { background: var(--tomato); }

.converged-loader__state-action:disabled {
  cursor: wait;
  opacity: .54;
}
