...
import type { Pattern, HistoryNote, PatternStorageStats } from '@/types/patterns'
...
export function logPatternDetected(pattern: Pattern) {
  ...
  console.log(`\n🎵 Note Sequence:`)
  const noteSequence = pattern.notes.map(note => 
    `${note.solfege.name}(${note.note})`
  ).join(' → ')
  console.log(`   ${noteSequence}`)
  
  console.log(`\n⏱️ Timing:`)
  console.log(`   Created: ${new Date(pattern.createdAt).toLocaleTimeString()}`)
  console.log(`   Average note duration: ${pattern.averageNoteDuration?.toFixed(0)}ms`)
  
  if (pattern.dominantScaleDegree) {
    console.log(`\n📈 Analysis:`)
    console.log(`   Dominant scale degree: ${pattern.dominantScaleDegree}`)
    console.log(`   Detection confidence: ${(pattern.detectionConfidence || 0).toFixed(2)}`)
  }
...
export function logHistoryState() {
  ...
  recentNotes.forEach((note, index) => {
    ...
    console.log(`   ${index + 1}. ${note.solfege.name}(${note.note}) - ${timeAgo < 60000 ? Math.floor(timeAgo/1000) + 's' : Math.floor(timeAgo/60000) + 'm'} ago`)
  })
  ...
  console.log(`   Last note time: ${data.lastNoteTime ? new Date(data.lastNoteTime).toLocaleTimeString() : 'Never'}`)
...
export function showPatternInsights() {
  ...
  console.log(`   Total patterns: ${stats.totalPatterns}`)
  console.log(`   Saved patterns: ${stats.savedPatterns}`)
  console.log(`   Average length: ${stats.averagePatternLength.toFixed(1)} notes`)
  ...
  const byInstrument = patterns.reduce((acc, pattern) => {
    acc[pattern.instrument] = (acc[pattern.instrument] || 0) + 1
    return acc
  }, {} as Record<string, number>)
  ...
  if (stats.mostPlayedPattern) {
    ...
    console.log(`   Key: ${stats.mostPlayedPattern.key} ${stats.mostPlayedPattern.mode}`)
...
export function showConfig() {
  ...
  console.log(`🧹 Auto-purge age: ${data.config.autoPurgeAge / (60 * 60 * 1000)}h`)
  ...
  console.log(`🔄 Detect on context change: ${data.config.detectOnContextChange}`)
  console.log(`💾 Auto-save interesting: ${data.config.autoSaveInterestingPatterns}`)
  console.log(`🧠 Auto-save threshold: ${data.config.autoSaveComplexityThreshold}`)
  console.groupEnd()
}
 ...
 */
export function updateConfig(newConfig: Partial<import('@/types/patterns').PatternDetectionConfig>) {
