...
| **Config Menu** | Configuration content inside the shared top Drawer | Drawer and Tabs are closed; define and adopt the interior, coordinating Sticker/Badge placement plus UIBeat's old-config migration before composition review |
...
6. **Config Menu formalization:** implement the accepted open Knob field, quiet groups, responsive density, separated local/global controls, Sticker-faced scene and preset actions, shared Buttons, and Ivory-only Sticker treatment in a focused real specimen before production adoption. Preserve the canonical grouped Blob schema, conditional visibility, persistence, and the future UIBeat migration boundary.
