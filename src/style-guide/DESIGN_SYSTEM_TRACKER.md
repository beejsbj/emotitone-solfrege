...
Keyboard remains **Under review**, not accepted or authorized for implementation. Explicit answers through Q51, except contradictory Q50, are recorded in the `BJS-35` Cockpit Thread; this section is the cross-session orientation spine, not a second chronological transcript.
...
- The generic config can request even `rowCount` values although rendering coerces them to odd. Near octave 1 or 8, current rendering filters unavailable candidates and returns fewer rows instead of restricting `mainOctave`; whether that edge behavior survives is Q50. Current QWERTY routing also targets main octave plus/minus one even when those rows are hidden, contradicting accepted visible-row routing.
...
- **Q50 — octave boundaries:** resolve the contradiction between the recommended symmetric rule and current behavior. Option A restricts `mainOctave` so every requested odd row count fits symmetrically. Current option B permits edge main octaves and filters unavailable rows, so `mainOctave: 1` with `rowCount: 3` displays only octaves 1 and 2. Burooj said both “agree” to A and “how it works currently makes sense,” so neither is accepted yet.
