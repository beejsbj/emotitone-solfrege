...
| CodeStrip | Unique | **Under review; next** | `components/uniques/CodeStrip.vue` and its real specimen hold the proposed visual grammar; production `LiveStrip.vue` holds the functionality CodeStrip succeeds and currently duplicates notation presentation. The specimen's obsolete `shape="glyph"` call is ignored by accepted `Note.vue`, so each note renders at the full 56x88px medium geometry and makes the claimed 36px row about 111px tall at the inspected guide width | Preserve common Note ownership but restore the old compact glyph aesthetic and production progressive fill; settle the inline-Note versus public-compound seam before formalization |
...
- Using the common Note primitive was the right lineage decision. The regression is that CodeStrip now receives Note's complete surfaced geometry instead of the compact inline identity the obsolete `shape="glyph"` call intended.
- Recover the earlier CodeStrip aesthetic: compact chromatic Note identities at rest, with muted duration, rest, punctuation, and grouping chrome. Do not retain the current oversized tile row.
...
- Next ownership gate: prefer a real compact glyph presentation on Note plus a CodeStrip-local token wrapper. Create a public compound only if a second real consumer needs the combined Note + duration + progress anatomy.
...
- CodeStrip's intended inline note treatment currently crosses a broken public seam: it passes an obsolete `shape="glyph"` attribute to Note, but accepted Note has no `shape` prop and therefore renders full geometry. Because Note owns musical identity and presentation, decide whether the strip consumes a real lower-layer inline Note presentation or intentionally uses a compact surfaced Note before repairing the lineage.
