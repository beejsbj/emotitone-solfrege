...
export class PatternService {
  ...
  recordNote(noteData: {
    ...
    if (this.history.length > this.config.maxHistorySize) {
      ...
      console.log(
        `📝 Trimmed ${excessNotes} old history notes (max: ${this.config.maxHistorySize})`
      );
    }

    console.log(
      `🎵 Recorded note: ${noteData.note} (${noteData.solfege.name}) in ${noteData.key} ${noteData.mode}`
    );
  ...
  updateNoteRelease(noteId: string, releaseTime: number): void {
    ...
    if (note) {
      ...
      console.log(
        `🎵 Updated note release: ${note.note} (duration: ${note.duration}ms)`
      );
    }
  }
  ...
  startNewSession(context?: {
    ...
    if (context) {
      console.log(
        `📍 Session context: ${context.key} ${context.mode} with ${context.instrument}`
      );
    }
  ...
  detectPatternsFromHistory(): Pattern[] {
    ...
    if (this.history.length < this.config.minPatternLength) {
      console.log(
        `📊 History too short for pattern detection (${this.history.length} < ${this.config.minPatternLength})`
      );
      return newPatterns;
    }
      ...
      if (lastNote) {
        ...
        if (
          ...
          if (currentGroup.length > 0) {
            ...
            console.log(
              `📊 Pattern boundary detected: ${
                contextChanged ? "context change" : "silence"
              } (${timeSinceLastNote}ms gap)`
            );
          }
          currentGroup = [note];
        } else {
  ...
  private resetSilenceTimer(): void {
    ...
    this.silenceTimer = setTimeout(() => {
      console.log(
        `🔇 Silence detected (${this.config.silenceThreshold}ms) - detecting patterns`
      );
      this.detectPatternsFromHistory();
  ...
  async loadDefaultPatterns(): Promise<void> {
    if (this.defaultPatternsLoaded) {
      ...
      console.log(
        "🎵 Current default patterns count:",
        this.getDefaultPatterns().length
      );
      return;
    }
    ...
    try {
      ...
      for (const pattern of defaultPatterns) {
        ...
        const deterministicId = `default_${pattern.name
          ?.toLowerCase()
          .replace(/\s+/g, "_")}`;
      ...
      console.log(
        `📚 Successfully loaded ${loadedCount} default patterns into pattern service`
      );
      console.log("📚 Total patterns in service:", this.patterns.size);
      console.log(
        "📚 Default patterns now available:",
        this.getDefaultPatterns().length
      );
    } catch (error) {
  ...
  purgeOldPatterns(): number {
    ...
    if (purgedCount > 0) {
      console.log(
        `🧹 Purged ${purgedCount} old patterns (older than ${
          this.config.autoPurgeAge / (60 * 60 * 1000)
        } hours)`
      );
    }
