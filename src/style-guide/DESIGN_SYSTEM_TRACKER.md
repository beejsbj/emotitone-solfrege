...
| Pattern Card | Guide and production have separate sources. The definition workbench composes accepted Card plus Bar Tape in collapsed and focused densities. Both densities use Card's top-right mark anchor for the ordinal; the rejected collapsed left-number rail is not Pattern Card anatomy | Inspect the corrected collapsed hierarchy, then accept or adjust Pattern Card before reconciling one authoritative source and preserving production actions/state |
