...
The helper resolves the checkout root, records the full `HEAD`, requires a clean checkout, and records the installed Bun version. The current production source reads one configurable Vite input, `VITE_PITCH_ANALYSIS_URL`. Because both Bun and Vite can load ignored dotenv files and exported variables with different precedence, an attributed build rejects that input when it is exported or defined with dotenv's `KEY=value` or `KEY: value` syntax in `.env`, `.env.local`, `.env.production`, or `.env.production.local`. Error messages name the input and source but never its value. The helper runs the repository's normal `bun run build` command with the child `NODE_ENV` explicitly fixed to `production`, then requires the same revision, a still-clean checkout, and the same unset-input policy. The manifest records that fixed mode. The helper hashes `dist/index.html`, every emitted JavaScript and CSS file, and identifies the direct JavaScript and stylesheet entry assets from the built HTML. Finally, it writes `dist/emotitone-build-identity.json`.
