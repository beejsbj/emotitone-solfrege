...
| Brass usage law | Doctrine, token UI, Sticker, tabs, knobs, buttons | Brass belongs with ink/ivory, not the brand palette | token/pattern | promoted as global `.brass` instrument-metal utility; Sticker badge is brass-only | Promotion Gate 2026-05-27 |
| Brand color roles | Doctrine, brand tokens, spine card, sticker | Decorative poster colors versus functional status drift | token/pattern | resolved: brand colors decorative only; semantic color aliases are legacy cleanup, not doctrine | Doctrine/Promotion Gate 2026-05-27 |
...
| Preset row | `PrimitiveSpineCard.vue` | Interactive row with status and buttons | primitive usage | pruned as over-extraction; represent as SpineCard with filled action | Taxonomy/Promotion Gate 2026-05-27 |
...
| Top drawer composition | `CompositionTopDrawer.vue` | Trigger row, panes, drawer shell, instrument/preset/settings content, app keyboard context | composition/product recipe | pruned from style guide; production `TopDrawer.vue` remains an app implementation concern | Taxonomy Gate 2026-05-27 |
...
- Brass as instrument metal coupled with ink and ivory, not the funk/poster brand palette.
...
| Kicker marker | `src/components/primatives/Kicker.vue` | Promoted dot+label anatomy, tone, dot geometry, density, inverse, and dot/label-only forms. Pruned unused alignment-demo CSS. | `PrimitiveKicker.vue` now composes `Kicker`; mono label is recorded as the resolved kicker/spec-marker typography exception. |
| Spine card | `src/components/primatives/SpineCard.vue` | Promoted brand spine panel, matching Kicker child, stamped headline, body copy, compact mode, bone tone surface, one-color rule, and ordinary action-slot usage in the specimen. | `PrimitiveSpineCard.vue` composes `SpineCard`; the separate `PresetRow` compound was pruned as over-extraction. |
...
| Code strip | `src/components/uniques/CodeStrip.vue` | Promoted singular notation row, note glyph modes, durations, rests, brackets/separators, lit glyph marker, density, wrapping, stacked duration, and duration-bar grammar. | Pattern compounds may compose this unique alongside primitives. |
...
| Drawer shell | `src/components/primatives/DrawerShell.vue` | Promoted bounded drawer frame, top/bottom anchor axis, scrim, torn handle, open/close behavior, optional resize snaps, stage push-down, and reduced-motion behavior. | `UniqueDrawer.vue` and production `TopDrawer.vue` compose or wrap `DrawerShell`; top-drawer style-guide composition was pruned. |
...
| Top drawer app region | `CompositionTopDrawer.vue` | User rejected the style-guide composition as premature app implementation. | deleted from style guide; revisit only during main-app implementation |
