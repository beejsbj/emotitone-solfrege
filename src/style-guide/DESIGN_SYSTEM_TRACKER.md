...
| CodeStrip action compound | Compound | **Accepted flush correction pending adoption** | `components/compounds/CodeStripActions.vue` is authoritative and shared by production and its real specimen. It defaults only its CodeStrip consumer to dense; the remaining 2px/4px dense inset is rejected production evidence | Remove the dense content inset entirely while preserving other density contexts and behavior |
