...
| Sticker + Badge | Primitive family; Badge is currently a Sticker variant | Visual definition is closed and Badge is brass-only. The filled Ivory Sticker is production-adopted by interactive Knob drag feedback; the public Knob source and real specimens exercise its Brass-to-Badge mapping, while current production has no Brass Knob. Outline and fill Stickers may compose one leading or trailing Mark through the same public source; the Spine Card specimen consumes that marked-content seam. The generic source API and guide still permit/show non-brass Badges | Remove the non-brass Badge drift while preserving Knob and marked-content consumption, then choose additional placements across Config, Instrument, Loading, and Pattern surfaces without reopening the visual design |
...
| Brand Cover | A unique source and guide specimen exist; visual/source/adoption gates remain open | Close the singular artifact; the Loading Screen is its planned production consumer |
| Spine Card | Source and guide specimen exist; the unit is being reworked in a separate lane. Its section marker now directly consumes Sticker's leading-Mark seam; `Kicker.vue` is retired | Do not otherwise redesign Spine Card in the Marks pass |
...
| **Loading Screen** | Complete startup/loading surface | Production currently uses the restored `LoadingSplash.vue`. The newer `LoadingScreen.vue` is guide-only and unaccepted; Brand Logo is ready and Brand Cover remains open, with marked Sticker/Spine Card reconciled if used |
...
| Brand/Loading | Brand Cover -> marked Sticker/Spine Card if consumed -> Loading Screen | Keyboard, Patterns, and top-menu work | Brand Logo is ready. Production replacement happens only after the composition is accepted; preserve the working LoadingSplash until then |
...
5. **Brand artifact:** close Brand Cover as the remaining singular unit ready for Loading Screen consumption; Brand Logo is ready.
