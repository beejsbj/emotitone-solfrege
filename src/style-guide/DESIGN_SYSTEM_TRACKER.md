...
| Bar Tape | Defined as the Pattern sequence compressed into a 4px musical-event timeline: chronological segments, duration-proportional widths, and Music Color-resolved fills. The existing source and guide still carry the superseded seven-seat scale-indicator model; production carries equivalent raw strip markup | Formalize the accepted timeline contract, route the real guide and production Pattern Card through it, and retire the unconsumed scale/size/state variants |
