...
| Card | Burooj named `Card.vue` as the shared soul, selected the first extracted CardShell recipe as its only visual basis, set Ivory as the default spine, retained the floating notched edge-label treatment across the family, and classified Spine Card as a Card variant. Label content remains consumer-owned; Spine Card moves its kicker metadata into this notch instead of rendering a duplicate internal Kicker. Compact, borderless, and light-inversion variants are rejected; source formalization and production adoption have not begun | Inspect one shared workbench candidate as the neutral Card, brand-spined Spine Card, and Ivory-spined Pattern Card; accept or adjust the family before implementation |
...
| Kicker + Spine Card | Burooj classified Spine Card as a Card variant and removed its internal Kicker: the kicker metadata moves into Card's notched edge label. Kicker remains an independent primitive whose wider adoption is unresolved | After Card acceptance, make Spine Card compose Card while preserving its one-brand-color spine, edge-label metadata, stamped headline, and body; audit Kicker independently before Loading Screen closure |
