...
function updateAmbientMode(mode: 'major' | 'minor') {
  ...
  gsap.to(gradientRef.value, {
    ...
    filter: isMinor ? 'brightness(0.6) saturate(0.8)' : 'brightness(1) saturate(1)',
    duration: 1.5,
    ease: 'power2.inOut',
  })
}
...
function createGradientBlob(colors: string[], x: number, y: number) {
  ...
  blob.style.cssText = `
    ...
    background: radial-gradient(circle at center, ${colors.join(', ')}, transparent);
    left: ${x}%;
    top: ${y}%;
...
function handleNotePlayed(event: CustomEvent) {
  const note: SolfegeNote = event.detail.note
  ...
  // Create particles if note has flecks
  if (note.flecks) {
    createParticles(15, note.colorGradient[0])
  }
}
