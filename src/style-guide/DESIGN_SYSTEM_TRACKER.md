...
| Card | Defined and authoritative as `Card.vue`; the guide mounts the real source. It owns the Ink-3 square shell, Ink-5 hairline, required notched edge label, 4px Ivory-default spine with a plain color input, optional mark/footer anchors, and flexible content. Colored Cards are not variants or a separate Spine Card family. The obsolete `CardShell.vue` and `SpineCard.vue` sources are retired. Compact, borderless, and light-inversion modes remain rejected | Production adoption waits for the accepted Pattern Card to compose Card while preserving its behavior and state ownership |
| Pattern Card | Definition remains open; Card approval did not approve this compound. Burooj has established its governing two-state idea: collapsed shows identity/metadata plus Bar Tape, while expanded/active replaces Bar Tape with CodeStrip and exposes Delete, Open in Strudel, and Copy Strudel Code actions. Both states compose the accepted Card shell and keep the ordinal in Card's top-right mark; Pattern List owns which card is active and any stack choreography. Existing guide actions/footer and production Keep behavior are reference evidence, not accepted anatomy | Inspect the dedicated two-state workbench; resolve exact metadata, action hierarchy, Keep disposition, and the separate deletion/opening behavior policy; then accept or adjust Pattern Card before source reconciliation |
...
| Kicker | Kicker remains an independent primitive. The rejected Spine Card concept is fully absorbed by ordinary Cards with colored spines; Card labels do not compose Kicker | Audit Kicker against real later-layer need before Loading Screen closure; adopt it independently, explicitly defer it, or remove it |
| Beat Indicator + Marks | Sources and guide specimens exist; acceptance and adoption are unresolved | Audit against real later-layer need; adopt, defer with reason, merge, or remove |
...
| **Loading Screen** | Complete startup/loading surface | Production currently uses the restored `LoadingSplash.vue`. The newer `LoadingScreen.vue` is guide-only and unaccepted; Brand Cover and Brand Logo are expected production uniques, with any Card/Kicker use resolved independently |
...
| Patterns | Card definition/adoption -> Pattern Card source reconciliation -> Pattern List real specimen/adoption -> PerformanceDeck | Brand/Loading and top-menu definition | Bar Tape is closed; PatternReel remains reference-only until reconciled; `DrawerKeyboard.vue` has one owner during integration |
...
| Brand/Loading | Brand Cover and Brand Logo in parallel -> Card/Kicker placement if consumed -> Loading Screen | Keyboard, Patterns, and top-menu work | Production replacement happens only after the composition is accepted; preserve the working LoadingSplash until then |
...
3. **Pattern foundation:** define Card's lowest reusable boundary, adopt it upward, then reconcile the two Pattern Card sources around Card and their shared, closed Bar Tape primitive while preserving production actions and store ownership.
