...
- CodeStrip Bar is the accepted adjacent compound. Deployed inspection showed that even the fully flush CodeStrip still read as a separate outlined object with three Buttons orbiting it. The accepted compound is one continuous translucent instrument-bar surface matching Control Bar, with no gaps or outline: brass Play/Stop, an unframed transparent dense CodeStrip, ink Backspace, and ivory Return all sit on the same plane. All actions remain icon-only 40px Buttons with accessible names and unchanged behavior.
- Control Bar is accepted, authoritative, mounted as a real specimen, and adopted in production at `f4d4225`. It composes the authoritative Knob primitive as Key, Mode, BPM, Octave, Rows, and the existing provisional Drawer Boolean in six equal-width slots across the full available width, with horizontal padding deprioritized and no scrolling at target mobile widths. Its existing translucent black/backdrop treatment now consumes the shared instrument-bar surface tokens alongside CodeStrip Bar. Drawer stays behaviorally and visually unchanged until the Drawer unit decides its final trigger and home.
...
CodeStrip Bar follows CodeStrip. Drawer follows as its own unit. `DrawerKeyboard.vue` becomes the production composition only after CodeStrip, CodeStrip Bar, Keyboard, and Drawer are defined. Then resume the remaining queue one unit per session and finish with a fresh lineage audit across the completed pass.
...
| CodeStrip Bar | Compound | **Accepted and closed** | `components/compounds/CodeStripBar.vue` is authoritative and is shared by production `DrawerKeyboard.vue` and real `style-guide/compounds/CompoundCodeStripBar.vue`. It composes accepted Buttons and an authoritative CodeStrip through its public unframed seam, and consumes the shared instrument-bar surface tokens also used by Control Bar | Preserve the shared translucent surface, zero gaps/frame/inset, component name, other CodeStrip contexts, and action behavior |
...
- CodeStrip Bar owns one continuous translucent black instrument-bar plane behind all four members, matching Control Bar rather than using the rejected opaque Ink-2 surface. The reusable surface and backdrop recipe belongs to shared tokens and both compounds consume it. CodeStrip Bar itself has no outline, outer padding, or inter-item gaps. Its CodeStrip consumer is dense, has zero content inset, and uses CodeStrip's explicit unframed transparent treatment so no inner rectangle divides the bar.
...
- The accepted CodeStrip Bar replaces the old mixed action bar. Do not fold Key, Mode, BPM, Octave, Rows, or Drawer into it. Control Bar is the accepted compound for those six current controls; Drawer remains provisional within it until the Drawer unit.
