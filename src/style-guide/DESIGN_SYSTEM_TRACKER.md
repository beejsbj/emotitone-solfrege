...
Keyboard remains **Under review**, not accepted or authorized for implementation. Explicit answers through Q55 are recorded in the `BJS-35` Cockpit Thread; this section is the cross-session orientation spine, not a second chronological transcript.
...
- **Q56 — glissando travel:** decide whether each pointer owns one Key at a time, releases it on crossing, attacks the next, and interpolates fast movement so crossed semitones are not skipped.
- **Q57 — drag boundary:** decide whether a held pointer releases on leaving Keyboard and resumes from the re-entered Key if it returns before pointer-up.
- **Q58 — glissando haptics:** decide whether direct-touch haptics pulse on every newly entered Key during a slide or only on initial contact.
- **Q59 — pointer and roving focus:** decide whether pointer/touch presses update the Key remembered by roving focus without forcibly showing keyboard-focus treatment during touch interaction.
- **Q60 — remapping while held:** decide what Keyboard-owned pointer/QWERTY sources do when `mainOctave`, `rowCount`, or active visibility changes while notes are held; centralized MIDI and sequencer ownership must remain unaffected.
