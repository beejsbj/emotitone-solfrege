...
- Button's definition is accepted: one icon-only momentary primitive, shaped as the punched-out circular paper chad—flat, filled, and carried above its surface by a small hard offset. Everyday material is ink/ivory; brass is an intentional per-consumer treatment, never state. Formalization and production adoption are active.
...
| Button | Primitive | **Accepted; formalization active** | The guide imports `components/primatives/IconButton.vue`; production uses duplicated `components/ui/IconButton.vue`; Undo and Send still use Knob Button. `ConfigPanel.vue` also misuses IconButton for two persistent toggles that belong to Boolean Knob | Create one authoritative source, mount a real specimen, adopt momentary consumers, and move persistent controls to Boolean Knob |
