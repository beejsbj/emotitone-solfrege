...
| CodeStrip | Unique | **Under review; next** | `components/uniques/CodeStrip.vue` and its real specimen hold the proposed visual grammar; production `LiveStrip.vue` holds the functionality CodeStrip succeeds and currently duplicates notation presentation. The specimen's obsolete `shape="glyph"` call is ignored by accepted `Note.vue`, so each note renders at the full 56x88px medium geometry and makes the claimed 36px row about 111px tall at the inspected guide width | Define the compact surfaced `glyph` variant of Note and preserve production progressive fill; then settle CodeStrip's remaining density and states before formalization |
...
- CodeStrip's intended compact note treatment currently crosses a broken public seam: it passes an obsolete `shape="glyph"` attribute to Note, but accepted Note has no `shape` prop and therefore renders full geometry. Repair that seam as a compact surfaced Note variant; do not replace it with bare text or manufacture a public CodeStrip-note compound.
