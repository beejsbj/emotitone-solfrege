...
- CodeStrip's complete visual definition is accepted and correctly adopted in production at `11c5736`, superseding the parallel-rendering mistake in `dd2ff7f`. Its surfaced `glyph` proportion is formalized at 3:4 with a CodeStrip-owned responsive block scale 20% below the comparison candidate. The one production Strudel mirror now mounts authoritative `CodeStrip.vue` as its blurred presentation; focus reveals the same mirror's editable raw Strudel source, and Strudel `onDraw` phase drives CodeStrip's Note/Chord/Rest progress and follow-scroll. The obsolete `showStrudelLine` fork and legacy CodeMirror token widgets/CSS are removed. Inline text and proportional distance remain retired.
...
- CodeStrip is now closed visually. The next definition gate is a separate compound that composes the upgraded CodeStrip with a small set of adjacent Buttons. Backspace/Undo, Play, and Send are current candidates; their final membership and arrangement remain open.
...
- define the CodeStrip action cluster's small Button membership and arrangement against the accepted production CodeStrip.
...
| CodeStrip | Unique | **Accepted and production-adopted** | `components/uniques/CodeStrip.vue` is authoritative, composes accepted Note and Chord, owns surfaced Rest and duration/density/temporal presentation, and is driven by the real `style-guide/uniques/UniqueCodeStrip.vue` specimen plus the single production Strudel mirror. `patterns/codeStripMirrorPresentation.ts` mounts that source inside CodeMirror while `LiveStrip.vue` retains runtime/editing and `liveCodeStripAdapter.ts` maps recorded timing plus Strudel phase into semantic tokens | Define the adjacent compact Button compound; maintain the shared source, mirror-focus, progress, and behavior-preservation tests |
| CodeStrip action cluster | Compound | Relationship accepted; definition pending | The current production `KeyboardActionBar.vue` is horizontally scrolling evidence to replace. Accepted Button is available below it; Backspace/Undo, Play, and Send are candidate children, not yet final anatomy | CodeStrip is closed; define the compound's membership and arrangement next |
...
- Burooj explicitly accepted this ownership and responsive-proportion contract on 2026-08-30, then chose 3:4 and requested the comparison candidate be made 20% smaller. `f6fc21a` formalized that choice in Note and CodeStrip, removed the obsolete `shape="glyph"` seam, made the row fill its host, and converted the guide workbench into a real-source specimen. Burooj accepted the complete corrected workbench on 2026-09-02. `dd2ff7f` mistakenly introduced a parallel recorded-note renderer beside an independently styled Strudel mirror; Burooj rejected that boundary, and `11c5736` corrected adoption by rendering the authoritative component through the mirror itself.
...
- Burooj accepted the complete live workbench on 2026-09-02, including its duration family, density family, Rest, Chord presentation, and temporal rhythm. CodeStrip is defined, its source is authoritative, its specimen is real, and the production Strudel mirror consumes that source. The production adapter groups overlap into clustered Notes, derives voicing and press ranks separately, and adds no visual grammar or chord recognition. Strudel's `onDraw` phase controls Note/Chord/Rest progress in the mounted CodeStrip—the progressive fill is the mirror's playback highlight. Focusing the same CodeMirror reveals editable raw Strudel; blur restores CodeStrip. There is no parallel display mode or `showStrudelLine` switch.
...
- CodeStrip is the unique successor to the old production LiveStrip visual. `11c5736` removes both the parallel supplement branch and the legacy CodeMirror token-widget recipe: one CodeMirror retains editing, playback, follow-scroll, and Strudel integration while mounting authoritative CodeStrip for its unfocused presentation. `LiveStrip.vue` remains only the runtime integration host, not a second visual system.
...
- Do not optimize the current horizontally scrolling action-bar Knob row. Replace its visual hierarchy with the next compound: accepted CodeStrip plus a deliberately small Button set. Its exact actions and layout remain unaccepted until that compound is defined.
...
- Production CodeStrip's local adapter supplies low-to-high `voicingOrder`, chronological `pressOrder`, and independent member progress for overlapping recorded notes, using the clustered display without attempting chord recognition. `FloatingPopup` still sends accumulated press order to Tonal and Tonal treats its first note as the inversion bass; `BJS-406` owns that separate bug. Chord naming remains a functional concern, not Chord visual-component ownership.
