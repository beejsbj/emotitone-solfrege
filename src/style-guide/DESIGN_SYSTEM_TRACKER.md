...
| Kicker + Spine Card | Sources and guide specimens exist; relationship is unresolved and is being reworked in a separate lane | Preserve its current source files and guide registration in the Marks pass; reconcile it with Loading Screen only in its owning lane |
| Mark family | Defined, authoritative, represented by a real specimen, and production-adopted by Stage particles. One renderer-neutral registry owns all twenty structural and musical glyphs plus particle optical scale; SVG and canvas consume the same paths | Preserve one family and one geometry owner. Loading Screen has a pre-existing copied triangle; if that provisional composition survives its separate rework, replace the copy with Mark there |
...
| **Stage** | The unified visual canvas: `UnifiedVisualEffects.vue` + `useUnifiedCanvas` and its canvas-owned layers | Its production particle layer now renders stable note-assigned Marks with shared geometry, particle optical scale, Music Color, and its existing physics; sparkle and mist are retired. The wider Stage definition remains pending. Stage is the canvas, not UIBeat; do not absorb DOM overlays or unrelated UI into it |
