...
| Joystick | Unique | Direction semantics, Analog/Digital brass treatments, relative drag, page edition, Knob-aligned face and visible-hardware diameter, shared analog dark well, canonical sheen, detent alignment, and Brass direction follower are defined, authoritative, represented by real fixed specimens, and production-adopted through Control Bar | Preserve automatic center, eight explicit directions, inert press, 5px two-axis relative drag, latch/held override, cancellation cleanup, native radio-grid access whose arrow/Home navigation selects and moves the roving tab stop, and fixed guide specimens |
