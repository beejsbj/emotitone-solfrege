...
| Mark family | Definition reopened by Burooj for an Astra-low drawing pass and common-symbol expansion. One renderer-neutral registry remains the geometry authority for SVG and canvas | Improve the cohesive hand-cut family, add a bounded set of recognizable musical omissions, and reverify display plus particle scales. Loading Screen's pre-existing copied triangle remains deferred to its own lane |
| Mark Sticker | Accepted replacement for the former Kicker unit: a compound that places Mark inside Sticker, optionally beside a short label | Formalize the compound and real specimen; remove Kicker from the guide while retaining only the temporary Spine Card adapter |
...
| **Stage** | The unified visual canvas: `UnifiedVisualEffects.vue` + `useUnifiedCanvas` and its canvas-owned layers | Particle geometry comes from Mark, but Burooj rejected stable interval assignments: each particle should choose randomly from the whole family while retaining Music Color and existing physics. The wider Stage definition remains pending. Stage is the canvas, not UIBeat; do not absorb DOM overlays or unrelated UI into it |
