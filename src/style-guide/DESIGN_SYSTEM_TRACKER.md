...
- **Accepted Knob definition:** Burooj accepted the reconciled Analog Ring + Digital Arc visual family on 2026-08-26 in the active BJS-35 T3/Codex session, after inspecting production captures, the live production row and motion, the mounted specimens, and the flagged Digital Button screenshot. Production Knob is the interaction and motion authority; the accepted change is a visual treatment, not a new control UX. The acceptance receipt changes documentation only and unblocks a separate formalization/migration slice.
...
- **Active implementation frontier:** formalize the accepted Knob family in the shared source and make both specimens demonstrate real production-driven roles and states; then migrate only the accepted `ring | arc` and semantic `brass | ivory` treatments into the existing production knob family while preserving its APIs, gestures, haptics, value logic, state, density, and microanimations. No component implementation occurred in the acceptance receipt.
- **Following frontier:** after the Knob formalization/migration receipt, begin a fresh CodeStrip visual grilling session, including its boundary and visual relationship with the action buttons/bar. Grill Drawer separately afterward. Only once CodeStrip + action bar, Keyboard, and Drawer are each defined should the current `DrawerKeyboard.vue` be reworked as their composition. Do not re-grill Note or Key. Music Color retains an independent later gate.
...
| Mounted reference/pending units | 8 | 8 | 4 | 3 | 1 | **24** |

One additional architecture unit is tracked but not mounted: the future **DrawerKeyboard composition** of CodeStrip + action bar, Keyboard, and Drawer. Across all 30 tracked units, 5 definitions are accepted: `Sticker`, `Note`, and `Key` are formalized, while the two mounted Knob visual units are one accepted family awaiting formalization. The remaining 25 still need a definition gate. The mounted Keyboard is a production-integrated reference, not an accepted definition. `Music Color Recipe` is a separately flagged token review, not authority to interrupt the current unit order.
...
| Knob — Analog | **Accepted** | Current shared `Knob.vue` and mounted specimen are reference implementations pending production-led correction | The separate production `components/knobs` family remains behavior, density, and motion authority | Formalize the accepted Ring treatment over the production role/state grammar, then migrate visuals without functional redesign |
| Knob — Digital | **Accepted** | Current shared `Knob.vue` and mounted specimen are reference implementations pending production-led correction; the mounted Button and Boolean presentations do not match the accepted definition | The separate production `components/knobs` family remains behavior, density, and motion authority | Formalize the accepted Arc treatment with full-circle Boolean and production-consistent Button, then migrate visuals without functional redesign |
...
2. Formalize the accepted Analog/Digital Knob family and its shared primitive, make the specimens production-driven, then migrate only the accepted visual treatment into the existing production knob family while preserving its behavior.
