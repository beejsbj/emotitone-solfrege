...
| Harmonic Geometry | Opt-in Unique canvas presentation with exactly two modes. Merge makes every blob one continuous body through its accepted sparse N-1 material topology. Web keeps bodies distinct while the analyzed full graph becomes contour-to-contour field filaments: perimeter strands are stronger, interior strands finer and dimmer, intersections fuse, and surviving roles stay stable through drift and release. `useBlobRenderer.ts` owns exact animated contours, `useBlobFieldRenderer.ts` owns both material treatments and capped/batched color accumulation, and `useHarmonicAnalysis.ts` plus `useHarmonicGeometryRenderer.ts` own the event-lived analysis, graph topology, and optional labels. Production and the real guide consume those seams; labels are independently optional and default off. Retired Outline/Center Only settings migrate to Merge/Web respectively; newly enabled geometry defaults to Merge | Inspect the deployed Web filament finish and maximum-density performance on PR #32. Preserve replay/lifecycle, Merge's accepted continuity, the 30k field-pixel budget, CodeStrip/Chord presentation, and feed the accepted two-mode result into later Stage definition |
