...
| Knob | Primitive deep module | Ring/Arc family is defined, authoritative, represented by real specimens, and production-adopted. Unpinned production Knobs share one app-load edition that alternates Ring/Arc between loads; explicit guide specimens stay fixed. Labels wrap without clipping, and formatted numeric displays expose at most two decimal places without changing emitted or persisted values. Brass uses the accepted instrument-metal sheen and glow; interactive Range/Options drag values use an Ivory Sticker in production and the accepted brass Badge treatment where Brass is exercised. Config production adopts Brass only for its two Boolean enable controls, which have no drag follower. Boolean Knob shares the promoted `--dur-bounce` / `--ease-bounce` elastic release with non-brass Buttons | Keep one public seam, page-wide edition consistency, viewport-safe gesture feedback, Reduced Motion stillness, and Range/Boolean/Options behavior; accepted follower/rip motion does not close the Motion collection |
...
| **Config Menu** | Configuration content inside the shared top Drawer | Defined, authoritative in production `ConfigPanel.vue`, mounted directly by the focused `/style-guide/config-menu` specimen, visually accepted, and production-adopted. The visible top-menu chrome now favors workhorse Ink and Ivory instead of a ladder of near-gray intermediates. Persistent settings are open Ivory Knob fields under quiet groups; Brass sheen and glow are reserved for the global Visuals and active-section Boolean masters; local Section controls sit with the active section; global utilities remain in the top rail; momentary icon actions use Button; Scenes and Presets use Ivory Sticker faces inside native buttons; Badge stays absent. Labels wrap cleanly, displayed numeric precision is capped at two places, and the shared rail/content gestures expose offscreen tabs without compromising vertical or Knob gestures. Preserve the canonical grouped Blob schema, conditional visibility, persistence, and future UIBeat migration boundary |
