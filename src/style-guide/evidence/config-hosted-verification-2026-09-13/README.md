...
`contract-probes-4997c46/` supersedes the assertion-probe JSON for the claims it repeats. The production Look receipt compares the complete kept config to the baseline with exactly the 35 source-defined Luminous-owned leaves overlaid across Blobs, Ambient, Particles, Strings, and Hilbert Scope. This also requires every field outside that allowlist—including other fields in those five sections and all Keyboard, Code Strip, dynamic-color, UIBeat, and other config—to remain structurally unchanged. Its live Stage snapshots include `aria-pressed` for the Stage master and boolean controls as well as the displayed range and option values. The production reload receipt requires a transient status and changed live Stage snapshot, then proves Discard restores the kept snapshot without changing persisted data. The guide receipt proves Soft Preview changes those live controls, Luminous Preview changes the baseline and differs from Soft, and Keep retains that Luminous preview while the ephemeral guide store remains isolated after each 500 ms save debounce.
