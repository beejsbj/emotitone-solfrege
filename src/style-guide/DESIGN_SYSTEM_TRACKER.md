...
- Chord is now a required public compound and the next lower dependency to define. CodeStrip needs fused and zero-gap clustered presentations; a future Keyboard chord row will consume fused + symbol only. Fused Chord owns one surface whose member-note color bands fill independently from bottom to top.
...
| Chord | Compound | **Relationship and core presentations accepted; visual definition pending** | Two real consumers are planned: CodeStrip compares fused and zero-gap clustered Chords, while a future Keyboard chord row uses fused + symbol only. Fused Chord is one surface with member-note gradient bands; clustered Chord composes touching Notes. Each member band progresses independently bottom-to-top so attack/release remains legible | Build a definition workbench for fused/clustered structure, symbol/member-note identity, responsive proportion, gradient order, and articulation states before source implementation |
