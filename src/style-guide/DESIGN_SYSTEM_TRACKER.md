...
| Mark family + Beat Indicator | Defined together. Mark is one primitive family containing structural and musical glyphs; Beat Indicator is a compound that selects and animates Marks directly. Existing sources/specimens still reflect the older independent-square split | Move all geometry into one renderer-neutral Mark authority, make the SVG primitive and Beat compound consume it, then rebuild their real specimens |
...
| **Stage** | The unified visual canvas: `UnifiedVisualEffects.vue` + `useUnifiedCanvas` and its canvas-owned layers | Particle lineage is defined: particles render assigned Marks with Music Color and existing physics; sparkle and mist retire. The wider Stage definition remains pending. Stage is the canvas, not UIBeat; do not absorb DOM overlays or unrelated UI into it |
