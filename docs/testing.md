...
The first cleanup pass's Mac run passed 1,661 runtime tests across 148 files, with one intentionally skipped benchmark and no failures or collection errors. The second pass (see `docs/testing-audit/2026-09-23/README.md`) removed dead utilities and duplicate cases and moved the benchmark to `bench:persistence`, ending at 1,562 passing tests across 146 files with none skipped. The resource figures below are from the first pass. `bun run test:verify` passed all four subprocess checks. `bun run build` passed, including the bounded typecheck; existing bundle-size and stale Browserslist warnings remain. The full runtime run took 30.54 seconds and peaked at 864.0 MiB sampled aggregate RSS; the build took 16.53 seconds and peaked at 1128.0 MiB. Process-tree RSS was sampled every 200 ms and can double-count shared pages or miss brief peaks. These were practical runs, not a cache-controlled benchmark. Validation and changes were local; the bjslab checkout was not updated by this work.
