...
| **Loading Screen** | Complete startup/loading surface | Production currently uses the restored `LoadingSplash.vue`. The newer `LoadingScreen.vue` is guide-only and unaccepted; Brand Cover and Brand Logo are expected production uniques, with marked Sticker reconciled if used |
...
| Brand/Loading | Brand Cover and Brand Logo in parallel -> marked Sticker if consumed -> Loading Screen | Keyboard, Patterns, and top-menu work | Production replacement happens only after the composition is accepted; preserve the working LoadingSplash until then |
