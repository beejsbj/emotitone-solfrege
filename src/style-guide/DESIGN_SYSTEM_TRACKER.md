...
| Keyboard | One production compound and controlled guide specimen. Its permanent chord row renders one fused playable chord per active 5/6/7/12-note scale degree, keeps each held face on its attacked snapshot while later owners attack live Harmony, normalizes extreme voicings into MIDI range, and keeps borrowed members colored. Native pointer, touch, keyboard, click-only, and mirrored-unison owners release independently. Broader density remains unaccepted | Inspect the existing variable-cardinality workbench and accept or adjust only the broader gap, row hierarchy, inset, typography, overlap, and variation-amplitude matrix |
