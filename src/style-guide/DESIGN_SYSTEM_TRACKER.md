...
| **Instrument Picker** | Instrument-selection content inside the shared top Drawer | Defined, authoritative in production `InstrumentSelector.vue`, mounted directly as the focused real guide specimen, and production-adopted. Every visible choice is a real Sticker inside an invisible native button: Ivory outline for available/ready, dimmed Ivory outline for cold, filled Ivory for current, and filled brass for warming. Badge has no role here | Preserve the shared Drawer/Tabs frame, search and category behavior, controlled/store selection flows, warmup/error handling, audio, accessibility semantics, and Sticker-owned visible choice material |
...
5. **Config Menu definition:** Instrument Picker is closed with Sticker choices and no Badge. Compare the remaining Config interior against `/style-guide/`; define its interior grammar and decide its own Sticker/Badge placement without redesigning behavior or pre-empting UIBeat's config migration.
