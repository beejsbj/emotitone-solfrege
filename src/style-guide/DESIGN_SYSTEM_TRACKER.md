...
| Patterns | Pattern Card source reconciliation -> Pattern List real specimen/adoption -> PerformanceDeck | Brand/Loading and top-menu definition | Bar Tape is closed; PatternReel remains reference-only until reconciled; `DrawerKeyboard.vue` has one owner during integration |
...
3. **Pattern foundation:** reconcile the two Pattern Card sources around their shared, closed Bar Tape primitive while preserving production actions and store ownership.
