...
5. **Config Menu definition:** Instrument Picker is closed with Ivory-only Sticker choices, ready-first ordering, shared instrument icons, and no Badge. Compare the remaining Config interior against `/style-guide/`; define its interior grammar and decide its own Sticker/Badge placement without redesigning behavior or pre-empting UIBeat's config migration.
