...
| Card | `CardShell.vue` is an extracted guide-only editorial panel with required title plus optional label/body/mark; it has no production consumer, while both Pattern Card sources own separate shells | Define whether Card is a flexible structural surface or an editorial content template, then make a real higher layer consume the accepted boundary before Pattern Card closes |
