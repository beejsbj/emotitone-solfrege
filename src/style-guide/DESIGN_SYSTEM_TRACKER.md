...
- Button is one icon-only primitive. It will absorb the current Knob Button's momentary action role; Boolean remains a Knob. Its accepted visual metaphor is the punched-out circular paper chad: flat, filled, and carried above its surface by a small hard offset, not an aperture, off-cut poster shape, dome, or bezel. Everyday material is ink/ivory; brass is an intentional per-consumer material treatment, not state. Consolidated acceptance remains pending.
...
| Button | Primitive | **Definition assembled; acceptance pending** | The guide imports `components/primatives/IconButton.vue`; production uses duplicated `components/ui/IconButton.vue`; Undo and Send still use Knob Button. `ConfigPanel.vue` also misuses IconButton for two persistent toggles that belong to Boolean Knob | Accept the consolidated definition before implementation |
...
- Proposed consolidated state treatment: hover changes paper contrast; press collapses the hard offset and rebounds; loading uses tone-matched perimeter motion rather than automatic brass; disabled dims and removes motion. Consolidated acceptance remains required before implementation.
