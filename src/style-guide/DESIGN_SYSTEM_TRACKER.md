...
| CodeStrip Bar | Compound | **Accepted; surface correction in progress** | `components/compounds/CodeStripBar.vue` is authoritative and is shared by production `DrawerKeyboard.vue` and real `style-guide/compounds/CompoundCodeStripBar.vue`. It composes accepted Buttons and an authoritative CodeStrip through its public unframed seam | Replace its rejected opaque Ink-2 rail with the same shared translucent instrument-bar surface as Control Bar; preserve zero gaps/frame/inset, component name, other CodeStrip contexts, and action behavior |
| Control Bar | Compound | **Accepted and closed pending Drawer reassignment** | `components/compounds/ControlBar.vue` is the controlled authoritative source; `style-guide/compounds/CompoundControlBar.vue` drives that source and `DrawerKeyboard.vue` binds the same seam to existing stores. The retired `LegacyKeyboardControls.vue` has no remaining source references | Preserve six equal-width, non-scrolling slots and existing mutations; reconsider only the provisional Drawer control during the Drawer unit |
