    ...
    if (
      ...
      Object.keys(section).forEach((key) => {
        ...
        if (
          ...
          // Ensure number values are not NaN
...
const getNumberMin = (sectionName: string, key: string): number => {
  const min = configMetadata[sectionName]?.[key]?.min;
  return (typeof min === 'number' && !isNaN(min)) ? min : 0;
};

const getNumberMax = (sectionName: string, key: string): number => {
  const max = configMetadata[sectionName]?.[key]?.max;
  return (typeof max === 'number' && !isNaN(max)) ? max : 100;
};

const getNumberStep = (sectionName: string, key: string): number => {
  const step = configMetadata[sectionName]?.[key]?.step;
  return (typeof step === 'number' && !isNaN(step)) ? step : 0.1;
};
