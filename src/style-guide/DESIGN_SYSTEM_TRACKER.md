...
- Chord is now a required public compound and the next lower dependency to define. A real source candidate and guide workbench exist: CodeStrip can compare fused and zero-gap clustered presentations, while a future Keyboard chord row will consume fused + symbol only. Fused Chord owns one surface whose member-note color bands fill independently from bottom to top. Exact proportion and final visual treatment remain unaccepted until Burooj inspects the workbench.
...
- define Chord as CodeStrip's next lower dependency, then finish CodeStrip's duration, rest, density, and temporal-state definition before production adoption.
...
| Chord | Compound | **Relationship and core presentations accepted; visual candidate under review** | `components/compounds/Chord.vue` is the single source candidate and `style-guide/compounds/CompoundChord.vue` mounts it in a real workbench. Fused is one continuous surface with runtime member-note color bands; clustered composes touching authoritative Notes. Controlled member progress fills independently bottom-to-top. No production consumer has adopted it | Inspect and adjust fused/clustered identity, compact/balanced/wide proportion, band treatment, labels, and articulation motion; accept the visual definition before CodeStrip adoption |
...
- Chord is a public compound rather than a CodeStrip-local group. Its fused presentation is one continuous gradient surface; its clustered presentation composes zero-gap Notes. Symbol versus member notes is a separate identity axis. The future Keyboard chord row consumes fused + symbol only.
- Fused Chord maps one color band to each member note and animates those bands independently from bottom to top, preserving individual attacks and releases within one outer surface. Clustered Chord lets each touching Note expose the same articulation directly.
...
### Chord — definition workbench

- `components/compounds/Chord.vue` is the provisional public source. Its independent axes are `fused | clustered`, `symbol | members`, and `compact | balanced | wide`; consumers provide member-note data and controlled progress values from 0 to 1.
- Fused owns one clipped surface divided into runtime music-color bands. Clustered composes real `Note proportion="glyph"` instances at zero layout gap and adds only Chord-local temporal overlays.
- Every member progresses independently from bottom to top with the existing LiveStrip-like 72ms linear response. Chord owns no playback clock, store, audio, haptics, or input behavior.
- The guide owns a looping rolled-attack hero and freezes it at a representative static state under Reduced Motion. Static rows compare simultaneous attack, rolled attack, staggered release, partial progress, both structures, both identities, and three responsive proportion candidates.
- Live browser evidence on 2026-08-30 found no component overflow at 320 or 390 CSS px; desktop, 390px, and 320px captures kept fused visually continuous and clustered Notes touching. A 320px measurement resolved the hero at 132x44px and the compact/balanced/wide tetrads at about 82x27, 108x36, and 132x44px. Over 320ms the live hero bands changed independently, proving the specimen drives the public controlled-progress seam.
- This is not visual acceptance or production adoption. Burooj still needs to judge the proportions, fused band weighting, label treatment, and choreography in the live workbench.
