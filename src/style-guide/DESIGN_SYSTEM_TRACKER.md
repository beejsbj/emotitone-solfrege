...
- Button is accepted, formalized, mounted as a real specimen, and adopted in production. Its corrected finish has no faux inset outline, uses recognizable centered icons, defaults production brass to canonical sheen + glow, and keeps full geometry-token studies explicitly specimen-only.
...
- Resting and pressed Buttons have no border or faux inset ring. Keyboard focus and forced-colors outlines remain accessibility exceptions. Button owns a recognizable icon scale of roughly 60% of the face and true centered SVG boxes; reassess individual optical imbalance only after the corrected scale is visible.
- The style guide may apply complete geometry-token recipes—clip, tilt, radius, and depth/shadow—to real Button instances as explicitly exploratory specimen evidence. These studies do not change the accepted circular production shape or add geometry variants to the public API.
...
- Visual correction and verification at `90b13f3` / `c1b7ab2`: 34/34 focused tests, type-check, and production build pass. Live guide evidence confirmed all normal borders at 0px; 19.2/24/28.8px icons for 32/40/48px faces; exact centered SVG boxes; distinct flat, sheen, glow, and sheen + glow brass; and tile/offcut/tab/rip/rounded studies combining clip or radius with tilt and shadow tokens. Live production resolved Undo/Send to 63px faces with 37.8px centered icons, while Send used the canonical gradient, animated sheen, and 14px glow without `aria-pressed`. Disabled production-default brass computed to `animation: none` and 35% opacity. Screenshot capture failed and is not claimed; DOM and computed-style evidence plus a fresh lineage re-audit passed.
