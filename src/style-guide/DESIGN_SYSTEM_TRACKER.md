...
| CodeStrip Bar | Compound | **Accepted rename and unity correction pending adoption** | Current `components/compounds/CodeStripActions.vue` and its production/specimen consumers are implementation evidence to rename. Its inner CodeStrip remains visibly framed and the gaps expose the page surface, splitting the compound | Rename the source/specimen/tests to CodeStrip Bar; give the bar one continuous Ink-2 surface; consume CodeStrip unframed and transparent; remove inter-item gaps; preserve actions and other CodeStrip contexts |
...
- The accepted CodeStrip Bar replaces the old mixed action bar and supersedes the temporary CodeStrip Actions name. Do not fold Key, Mode, BPM, Octave, Rows, or Drawer into it. Control Bar is the accepted compound for those six current controls; Drawer remains provisional within it until the Drawer unit.
