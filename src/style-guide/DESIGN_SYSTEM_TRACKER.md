...
- CodeStrip's surfaced `glyph` proportion is accepted and formalized at 3:4 with a CodeStrip-owned responsive block scale 20% below the comparison candidate. Its real source and guide workbench compose accepted Note and Chord, own a surfaced Rest, expose stacked-text, time-signature-mark, and hidden duration treatments, compare three densities, and accept controlled 0–1 temporal progress. Inline text and proportional distance are retired. CodeStrip remains the unique production successor to LiveStrip, not one half of an action-bar unit; the corrected workbench awaits final visual selection before production adoption.
...
- Inline textual duration and proportional-distance duration were explicitly retired on 2026-09-01. The remaining treatments are stacked text, time-signature-aware proportional marks, and hidden. Bar marks derive their subdivision count from the meter and appear for Note, Chord, and Rest; Rest still never prints a textual duration.
...
- `durationMode="stacked" | "bar" | "hidden"` applies one presentation across the sequence. `timeSignature` defaults to `4/4`; bar mode divides a full cycle into denominator-aware sixteenth-note marks and makes each notated beat boundary taller. Rest participates in these meter marks while remaining textually untagged. `density="dense" | "default" | "spaced"` changes host scale, padding, and gaps without changing notation anatomy.
- Live computed-style evidence on 2026-09-01 found the 339x48 hero contained with no overflow and composing two Notes, one Chord, and two Rests. Note covers resolved to Ink with top-origin retraction; fused Chord bands retained runtime music colors with bottom-origin scale; Rest resolved to Ink/offcut/paper sheen with an Ivory bottom-origin fill and contrast-reversing mark. After duration refinement, the workbench mounted only stacked, bar, and hidden modes; equal events produced 15 total marks in 4/4 and 12 in 3/4, Rest owned one mark for its `@0.0625` duration, and every inspected row remained contained. Screenshot capture and a fresh narrow resize failed and are not claimed.
...
- Chord preserves the supplied `members` array as its left-to-right fused-band or clustered-Note order. Each member's progress is independent of that order, so a future adapter can keep voicing order stable while reflecting different press times or releases. Chord itself does not detect, name, sort, or temporally group notes.
...
- Production chord grouping needs a separately scoped adapter decision before Chord adoption: pitch/voicing order and temporal press order must not be conflated. `Chord.vue` deliberately preserves caller order; `StrudelNotation` sorts captured notes by `pressTime` then pitch, while `FloatingPopup` sends accumulated press order to Tonal and Tonal treats its first note as the inversion bass. Canonical low-to-high voicing and chord-name detection are functional concerns, not Chord visual-component ownership.
