...
- Chord is now a required public compound and the next lower dependency to define. Its display contract is coupled: `symbol` is always one fused surface, while `notes` is always a zero-gap cluster of authoritative Notes; invalid fused-members and clustered-symbol combinations do not exist. The fused surface consumes Note's paper sheen, shadow, and whole-surface geometry family. Progress starts at Ink and reveals unchanged music colors independently from bottom to top. Exact proportion and final visual acceptance remain open pending Burooj's live reinspection.
...
| Chord | Compound | **Display contract accepted; corrected visual candidate under review** | `components/compounds/Chord.vue` is the single source candidate and `style-guide/compounds/CompoundChord.vue` mounts it in a real workbench. `display="symbol"` is one fused paper surface; `display="notes"` is a zero-gap cluster of authoritative Notes. Controlled member progress reveals unchanged music color over an Ink base. No production consumer has adopted it | Reinspect the paper-cut material, geometry, type scale, proportions, and articulation in the live workbench; accept the visual definition before CodeStrip adoption |
...
- This is not final visual acceptance or production adoption. Burooj still needs to reinspect the corrected workbench and judge the proportions, geometry, label treatment, and choreography.
