...
| Harmonic Geometry | Opt-in Unique canvas presentation. Merge's shared-field material character and all-distance continuity are accepted; `useBlobRenderer.ts` owns exact animated contours, `useBlobFieldRenderer.ts` owns the material field, and production plus the real guide consume those seams. Labels are independently optional and default off. The rounded long-range filament and Outline finish remain under review | Inspect the deployed rounded far-distance Merge taper and Outline, then accept or adjust only that finish. Preserve replay/lifecycle, sparse N-1 topology, the capped field budget, CodeStrip/Chord presentation, and feed the accepted result into later Stage definition |
