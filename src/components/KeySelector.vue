<template>
  <div
    class="key-selector rounded-sm transition-all duration-300 absolute inset-0"
    :class="{
      '-translate-y-[60%]': showKeySelector,
    }"
  >
    <div class="space-y-6">
      <button @click="handleShowKeySelector">
        {{ showKeySelector ? "Hide Key Selector" : "Select Key" }}
      </button>
      <!-- Circle of Fifths -->
          ...
          <g :transform="`rotate(${-rotationAngle} 160 160)`">
            <g v-for="(key, index) in circleOfFifths" :key="`major-${key}`">
              ...
              <text
                ...
                :y="
                  160 + 120 * Math.sin(((index * 30 - 90) * Math.PI) / 180) + 5
                "
                text-anchor="middle"
          ...
          <g :transform="`rotate(${-rotationAngle} 160 160)`">
            <g v-for="(key, index) in relativeMinors" :key="`minor-${key}`">
              ...
              <text
                ...
                :y="
                  160 + 80 * Math.sin(((index * 30 - 90) * Math.PI) / 180) + 4
                "
                text-anchor="middle"
...
function handleShowKeySelector() {
  showKeySelector.value = !showKeySelector.value;
}
