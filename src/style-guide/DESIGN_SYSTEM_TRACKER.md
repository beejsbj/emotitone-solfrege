...
| Harmonic Geometry | Opt-in Unique canvas presentation. Merge's shared-field material character and all-distance continuity are accepted; `useBlobRenderer.ts` owns exact animated contours, `useBlobFieldRenderer.ts` owns the material field, and `useHarmonicAnalysis.ts` owns the event-lived analysis snapshot consumed by canvas labels plus UnifiedVisualEffects' synchronized accessible text. Production and the real guide consume those seams; labels are independently optional and default off. The retired `FloatingPopup.vue` source and its obsolete transition-duration control are removed. The rounded long-range filament and Outline finish remain under review | Inspect the deployed rounded far-distance Merge taper and Outline, then accept or adjust only that finish. Preserve replay/lifecycle, sparse N-1 topology, the capped field budget, CodeStrip/Chord presentation, and feed the accepted result into later Stage definition |
...
1. **Harmonic Geometry deployed acceptance:** inspect the rounded all-distance Merge and Outline on PR #32. If accepted, close its exact treatment and retain it as constrained Stage input; if adjusted, keep the correction inside the authoritative field renderer without reopening its material, topology, labels, or lifecycle.
