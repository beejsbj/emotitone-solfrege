...
| Harmonic Geometry | Opt-in Unique canvas presentation with exactly two modes. Merge makes every blob one continuous body through its accepted sparse N-1 material topology. Web keeps bodies distinct while the analyzed full graph becomes field filaments: perimeter strands are stronger, interior strands finer and dimmer, intersections fuse, and surviving roles stay stable through drift and release. `useBlobRenderer.ts` owns lifecycle, exact animated contours, breathing, and edge motion; Blob blur and color-preserving glow now finish ordinary, Merge, and Web bodies consistently. `useBlobFieldRenderer.ts` owns the capped/batched shared field, relationship morphology, and Harmony softness/threshold; Web attaches its filaments to stable rendered radii so contour vibration remains body motion rather than shaking the graph. `useHarmonicAnalysis.ts` plus `useHarmonicGeometryRenderer.ts` own the event-lived analysis, graph topology, and optional labels. Production and the real guide consume those seams; Web remains in the field before it has a relationship, labels are independently optional and default off, retired Outline/Center Only settings migrate to Merge/Web respectively, and newly enabled geometry defaults to Merge | Inspect the deployed one-note-to-Web transition, filament stability, glow/blur continuity, and maximum-density performance on PR #32. Preserve replay/lifecycle, Merge's accepted continuity, the 30k field-pixel budget, CodeStrip/Chord presentation, and feed the accepted two-mode result into later Stage definition |
...
1. **Harmonic Geometry deployed acceptance:** inspect the fused-filament Web and its maximum-density performance on PR #32 beside the accepted all-distance Merge. If accepted, close the two-mode treatment and retain it as constrained Stage input; if adjusted, keep the correction inside the authoritative field renderer without reopening its material ownership, labels, or lifecycle.
